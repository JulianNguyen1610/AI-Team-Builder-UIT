...
from config import (
    ...
    TACTICAL_PROFILES, 
    STRICT_RIGHT_SIDED_POSITIONS, STRICT_LEFT_SIDED_POSITIONS # <--- Đã có thể import từ config
)

# --- CÁC HẰNG SỐ CẤU HÌNH CỤC BỘ ---
PASSING_THRESHOLD = 75          
INVERTED_FB_THRESHOLD = 74      
CB_MIN_HEIGHT = 183             
MIN_WINGER_PACE = 83            
...
def calculate_ml_suitability_score(player_row, required_position_key, models_dict, tactic_name='Balanced (Cân bằng)'):
    ...
    # --- LOGIC ĐIỂM THƯỞNG CHIẾN THUẬT ---
    weights = TACTICAL_PROFILES.get(tactic_name, {})
    if weights:
        tactical_score = 0
        weight_sum = 0
        for stat, weight in weights.items():
            ...
            if pd.notna(val):
                tactical_score += float(val) * weight
                weight_sum += weight
        
        if weight_sum > 0:
            normalized_bonus = (tactical_score / weight_sum) / 20 
            predicted_score += normalized_bonus
